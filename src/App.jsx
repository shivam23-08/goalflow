...
function generateHeatmapData() {
  ...
  for (let i = 364; i >= 0; i--) {
    ...
    data[key] = Math.random() > 0.4 ? Math.floor(Math.random() * 5) : 0;
  }
  return data;
}
...
const MOCK_GOALS = [
  { id: 1, title: "Read 30 pages", desc: "Finish The Pragmatic Programmer", category: "Study", priority: "High", target: 30, unit: "pages", progress: 22, type: "daily", deadline: new Date(Date.now() + 86400000).toISOString(), completed: false, recurring: true, createdAt: new Date(Date.now() - 86400000 * 3).toISOString() },
  { id: 2, title: "Morning Run", desc: "5km outdoor run", category: "Fitness", priority: "High", target: 5, unit: "km", progress: 5, type: "daily", deadline: new Date(Date.now() + 3600000 * 2).toISOString(), completed: true, recurring: true, createdAt: new Date(Date.now() - 86400000 * 10).toISOString() },
  { id: 3, title: "Ship Feature X", desc: "Complete auth module for project", category: "Work", priority: "Critical", target: 8, unit: "hours", progress: 6, type: "weekly", deadline: new Date(Date.now() + 86400000 * 3).toISOString(), completed: false, recurring: false, createdAt: new Date(Date.now() - 86400000 * 2).toISOString() },
  { id: 4, title: "Meditate", desc: "Mindfulness for focus", category: "Health", priority: "Medium", target: 20, unit: "minutes", progress: 20, type: "daily", deadline: new Date(Date.now() + 3600000 * 5).toISOString(), completed: true, recurring: true, createdAt: new Date(Date.now() - 86400000 * 15).toISOString() },
  { id: 5, title: "Save ₹5000", desc: "Monthly savings goal", category: "Finance", priority: "High", target: 5000, unit: "₹", progress: 3200, type: "monthly", deadline: new Date(Date.now() + 86400000 * 20).toISOString(), completed: false, recurring: true, createdAt: new Date(Date.now() - 86400000 * 10).toISOString() },
  { id: 6, title: "Sketch Daily", desc: "One sketch per day for creativity", category: "Creative", priority: "Low", target: 1, unit: "sketch", progress: 0, type: "daily", deadline: new Date(Date.now() + 86400000).toISOString(), completed: false, recurring: true, createdAt: new Date(Date.now() - 86400000 * 1).toISOString() },
];
...
const MiniBarChart = ({ data, color = "#6EE7F7", dark }) => {
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 40 }}>
      {data.map((v, i) => (
        <div key={i} style={{
          ...
          background: i === data.length - 1 ? color : dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)",
      ...
      <div style={{ display: "flex", gap: 3 }}>
        {weeks.map((week, wi) => (
          <div key={wi} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            {week.map((day, di) => (
              <div key={di} title={`${day.key}: ${day.count} goals`} style={{
...
export default function GoalTracker() {
  ...
  const [unlockedBadges, setUnlockedBadges] = useState(["first_goal", "streak_3", "streak_7"]);
  ...
  const productivityScore = Math.round((completedToday / Math.max(totalGoals, 1)) * 100);
  ...
  const toggleGoal = (id) => {
    setGoals(gs => gs.map(g => {
      if (g.id !== id) return g;
      const nowDone = !g.completed;
      if (nowDone) {
        setXP(x => x + 50);
        showNotif("🎯 Goal completed! +50 XP");
      }
      return { ...g, completed: nowDone, progress: nowDone ? g.target : g.progress };
    }));
  };
  ...
  const updateProgress = () => {
    ...
    if (isNaN(val)) return;
    setGoals(gs => gs.map(g => {
      if (g.id !== showProgressModal.id) return g;
      const newProg = Math.min(val, g.target);
      const done = newProg >= g.target;
      if (done && !g.completed) { setXP(x => x + 50); showNotif("🎯 Goal completed! +50 XP"); }
      return { ...g, progress: newProg, completed: done };
    }));
      ...
      <div style={{ minHeight: "100vh", background: dark ? "#0A0B0F" : "#F4F5F9", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif", padding: 16 }}>
        <style>{`
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          input::placeholder { color: ${dark ? "rgba(232,234,237,0.3)" : "rgba(26,27,46,0.3)"}; }
          ::-webkit-scrollbar { width: 5px; height: 5px; }
          ::-webkit-scrollbar-track { background: transparent; }
          ::-webkit-scrollbar-thumb { background: rgba(110,231,247,0.3); border-radius: 10px; }
          ...
          .btn-primary { background: linear-gradient(135deg, #6EE7F7, #A78BFA); border: none; color: #0A0B0F; font-weight: 700; cursor: pointer; border-radius: 12px; padding: 14px; font-size: 15px; width: 100%; transition: all 0.2s; font-family: 'DM Sans', sans-serif; letter-spacing: 0.3px; }
          .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(110,231,247,0.35); }
          .input-field { width: 100%; padding: 13px 16px; border-radius: 12px; border: 1.5px solid ${dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.1)"}; background: ${dark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)"}; color: ${dark ? "#E8EAED" : "#1A1B2E"}; font-size: 14px; font-family: 'DM Sans', sans-serif; outline: none; transition: border-color 0.2s; }
          .input-field:focus { border-color: #6EE7F7; background: ${dark ? "rgba(110,231,247,0.04)" : "rgba(110,231,247,0.06)"}; }
        `}</style>
            ...
            <div style={{ display: "flex", background: dark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)", borderRadius: 12, padding: 4, marginBottom: 28, gap: 4 }}>
              {["login", "signup"].map(s => (
                <button key={s} onClick={() => { setScreen(s); setAuthError(""); }}
                  style={{ flex: 1, padding: "9px 0", borderRadius: 9, border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, transition: "all 0.2s",
                    background: screen === s ? "linear-gradient(135deg, #6EE7F7, #A78BFA)" : "transparent",
                    color: screen === s ? "#0A0B0F" : dark ? "rgba(232,234,237,0.5)" : "rgba(26,27,46,0.5)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {screen === "signup" && (
                <div style={{ position: "relative" }}>
                  ...
                  <input className="input-field" style={{ paddingLeft: 42 }} placeholder="Full name" value={authData.name} onChange={e => setAuthData(d => ({ ...d, name: e.target.value }))} />
                </div>
              )}
              <div style={{ position: "relative" }}>
                ...
                <input className="input-field" style={{ paddingLeft: 42 }} placeholder="Email address" type="email" value={authData.email} onChange={e => setAuthData(d => ({ ...d, email: e.target.value }))} />
              </div>
              <div style={{ position: "relative" }}>
                ...
                <input className="input-field" style={{ paddingLeft: 42 }} placeholder="Password" type="password" value={authData.password} onChange={e => setAuthData(d => ({ ...d, password: e.target.value }))} />
              </div>
              {authError && <p style={{ color: "#FF6B6B", fontSize: 13, textAlign: "center" }}>{authError}</p>}
              <button className="btn-primary" style={{ marginTop: 4 }} onClick={screen === "login" ? handleLogin : handleSignup}>
                {screen === "login" ? "Sign In →" : "Create Account →"}
              </button>
              {screen === "login" && (
                <p style={{ textAlign: "center", fontSize: 13, color: dark ? "rgba(232,234,237,0.4)" : "rgba(26,27,46,0.4)", cursor: "pointer" }}
                  onClick={() => showNotif("📧 Reset link sent to your email!")}>
                  Forgot password?
                </p>
              )}
            </div>
          </div>
          <p style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: dark ? "rgba(232,234,237,0.25)" : "rgba(26,27,46,0.25)" }}>
            JWT-secured • End-to-end encrypted • GDPR compliant
          </p>
        </div>
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', sans-serif", color: C.text, display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input::placeholder, textarea::placeholder { color: ${dark ? "rgba(232,234,237,0.25)" : "rgba(26,27,46,0.25)"}; }
        ::-webkit-scrollbar { width: 5px; height: 5px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: rgba(110,231,247,0.2); border-radius: 10px; }
        @keyframes fadeIn { from { opacity:0; transform:translateY(16px); } to { opacity:1; transform:translateY(0); } }
        @keyframes slideDown { from { opacity:0; transform:translateY(-12px); } to { opacity:1; transform:translateY(0); } }
        @keyframes glow { 0%,100% { box-shadow: 0 0 20px rgba(110,231,247,0.15); } 50% { box-shadow: 0 0 35px rgba(110,231,247,0.3); } }
        ...
        .card:hover { border-color: rgba(110,231,247,0.15); transform: translateY(-1px); }
        ...
        .tab-active { color: #6EE7F7 !important; }
        .goal-item { animation: fadeIn 0.3s ease; }
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 100, background: dark ? "rgba(10,11,15,0.85)" : "rgba(244,245,249,0.85)", backdropFilter: "blur(20px)", borderBottom: `1px solid ${C.border}`, padding: "0 24px", display: "flex", alignItems: "center", height: 64, gap: 16 }}>
        ...
        {/* POMODORO */}
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: C.surface2, borderRadius: 10, padding: "7px 14px", cursor: "pointer" }} onClick={() => { setPomodoroActive(true); }}>
          ...
          <button className="btn" style={{ padding: "3px 9px", fontSize: 11, background: pomodoroRunning ? "rgba(255,107,107,0.15)" : "rgba(134,239,172,0.15)", color: pomodoroRunning ? "#FF6B6B" : "#86EFAC" }}
            onClick={e => { e.stopPropagation(); setPomodoroRunning(r => !r); }}>
            {pomodoroRunning ? "⏸" : "▶"}
          </button>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: C.surface2, borderRadius: 10, padding: "7px 14px" }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600 }}>Lv.{getLevel(xp)}</span>
          <div style={{ width: 50, height: 5, borderRadius: 3, background: C.border, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${getLevelProgress(xp)}%`, background: "linear-gradient(90deg, #6EE7F7, #A78BFA)", borderRadius: 3 }} />
          </div>
          <span style={{ fontSize: 12, color: C.muted }}>{xp} XP</span>
        </div>

        <button className="btn btn-ghost" style={{ padding: "8px", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setDark(d => !d)}>
          <Icon name={dark ? "sun" : "moon"} size={16} />
        </button>
        <button className="btn btn-ghost" style={{ padding: "8px", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => { setScreen("login"); }}>
        ...
        <nav style={{ width: 220, background: C.surface, borderRight: `1px solid ${C.border}`, padding: "24px 12px", display: "flex", flexDirection: "column", gap: 4, position: "sticky", top: 64, height: "calc(100vh - 64px)" }}>
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", borderRadius: 12, border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: activeTab === tab.id ? 600 : 400, textAlign: "left", transition: "all 0.18s",
                background: activeTab === tab.id ? "linear-gradient(135deg, rgba(110,231,247,0.12), rgba(167,139,250,0.08))" : "transparent",
          ...
          <div style={{ marginTop: "auto", padding: "16px 14px", borderRadius: 14, background: C.surface2, border: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: "#FCD34D" }}>{streak}-day streak!</span>
            </div>
            <div style={{ fontSize: 11, color: C.muted, lineHeight: 1.5 }}>
              Keep it up! Your best was {longestStreak} days 🏆
            </div>
          </div>
        </nav>
          ...
          {activeTab === "dashboard" && (
            <div style={{ animation: "fadeIn 0.4s ease" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
                <div>
                  <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 26, fontWeight: 700, letterSpacing: -0.5 }}>Good morning! 👋</h2>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 28 }}>
                {[
                  { label: "Today's Goals", value: `${completedToday}/${totalGoals}`, icon: "target", color: C.accent, sub: "completed" },
                  { label: "Productivity", value: `${productivityScore}%`, icon: "trending", color: "#86EFAC", sub: "score today" },
                  { label: "Current Streak", value: `${streak} days`, icon: "flame", color: "#FCD34D", sub: `best: ${longestStreak}` },
                  { label: "Total XP", value: xp.toLocaleString(), icon: "zap", color: "#A78BFA", sub: `Level ${getLevel(xp)} · ${getLevelTitle(getLevel(xp))}` },
                  { label: "Active Goals", value: goals.filter(g => !g.completed).length, icon: "refresh", color: "#FDA4AF", sub: "in progress" },
                  { label: "Badges", value: unlockedBadges.length, icon: "award", color: "#FCD34D", sub: `of ${BADGES.length}` },
                ].map((stat, i) => (
                  <div key={i} className="card" style={{ position: "relative", overflow: "hidden" }}>
                    <div style={{ position: "absolute", top: 16, right: 16, opacity: 0.12 }}>
                      <Icon name={stat.icon} size={40} color={stat.color} />
                    </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 24 }}>
                <div className="card">
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    ...
                    <span style={{ fontSize: 12, color: C.muted, background: C.surface2, padding: "4px 10px", borderRadius: 20 }}>{goals.filter(g => g.type === "daily").length} goals</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {goals.filter(g => g.type === "daily").map(goal => {
                      ...
                      return (
                        <div key={goal.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", borderRadius: 12, background: C.surface2, border: `1px solid ${goal.completed ? "rgba(134,239,172,0.2)" : C.border}` }}>
                          <button onClick={() => toggleGoal(goal.id)} style={{ width: 24, height: 24, borderRadius: 8, border: `2px solid ${goal.completed ? "#86EFAC" : C.border}`, background: goal.completed ? "#86EFAC" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s" }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                              ...
                              <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 20, background: `${color}18`, color: color, fontWeight: 600 }}>{goal.category}</span>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <div style={{ flex: 1, height: 5, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                                <div className="progress-bar-fill" style={{ height: "100%", width: `${pct}%`, background: goal.completed ? "#86EFAC" : `linear-gradient(90deg, ${color}, ${color}99)`, borderRadius: 3 }} />
                              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div className="card">
                    <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Weekly Progress</h3>
                      ...
                      {weeklyData.map((v, i) => {
                        ...
                        return (
                          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                            <div style={{ width: "100%", borderRadius: 4, height: `${(v / 7) * 64}px`, background: isToday ? "linear-gradient(180deg, #6EE7F7, #A78BFA)" : dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)", minHeight: 6, transition: "height 0.5s ease" }} />
                            <span style={{ fontSize: 9, color: isToday ? C.accent : C.muted, fontWeight: isToday ? 700 : 400 }}>{days[i]}</span>
                          </div>
                        );
                      })}
                    </div>
                    <p style={{ fontSize: 12, color: C.muted }}>Avg: {(weeklyData.reduce((a, b) => a + b) / 7).toFixed(1)} goals/day</p>
                  </div>
                  ...
                  <div className="card">
                    ...
                    {Object.entries(CATEGORY_COLORS).slice(0, 5).map(([cat, color]) => {
                      ...
                      return catGoals.length > 0 && (
                        <div key={cat} style={{ marginBottom: 10 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                            <span style={{ fontSize: 12, fontWeight: 500 }}>{cat}</span>
                            <span style={{ fontSize: 12, color: C.muted }}>{done}/{catGoals.length}</span>
                          </div>
                          <div style={{ height: 5, background: C.border, borderRadius: 3, overflow: "hidden" }}>
                            <div className="progress-bar-fill" style={{ height: "100%", width: `${pct}%`, background: color, borderRadius: 3 }} />
                          </div>
                        </div>
                      );
                    })}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: C.muted }}>
                    ...
                    {[0.05, 0.3, 0.5, 0.7, 1].map((o, i) => (
                      <div key={i} style={{ width: 12, height: 12, borderRadius: 2, background: `rgba(110,231,247,${o})` }} />
                    ))}
                    <span>More</span>
                  </div>
                </div>
                <Heatmap data={HEATMAP_DATA} dark={dark} />
              </div>
            </div>
          )}
            ...
            <div style={{ animation: "fadeIn 0.4s ease" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
                <div>
                  ...
                  <p style={{ color: C.muted, fontSize: 14, marginTop: 4 }}>{goals.length} total · {goals.filter(g => g.completed).length} completed</p>
                </div>
                <button className="btn btn-accent" style={{ padding: "11px 20px", fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}
                  onClick={() => { setEditGoal(null); setShowForm(true); }}>
                  <Icon name="plus" size={16} color="#0A0B0F" /> New Goal
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {[
                  ...
                  <select key={i} className="input" style={{ width: "auto", height: 40, paddingRight: 28, cursor: "pointer" }} value={f.value} onChange={e => f.setter(e.target.value)}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {filteredGoals.map(goal => {
                  ...
                  return (
                    <div key={goal.id} className="card goal-item" style={{ borderColor: goal.completed ? "rgba(134,239,172,0.2)" : isOverdue ? "rgba(255,107,107,0.2)" : C.border }}>
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                        <button onClick={() => toggleGoal(goal.id)} style={{ marginTop: 2, width: 26, height: 26, borderRadius: 8, border: `2px solid ${goal.completed ? "#86EFAC" : C.border}`, background: goal.completed ? "#86EFAC" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.2s" }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 10, padding: "3px 9px", borderRadius: 20, background: `${color}18`, color: color, fontWeight: 600 }}>{goal.category}</span>
                          ...
                          <p style={{ fontSize: 13, color: C.muted, marginBottom: 12, lineHeight: 1.5 }}>{goal.desc}</p>

                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            <div style={{ flex: 1, height: 7, background: C.surface2, borderRadius: 4, overflow: "hidden" }}>
                              <div className="progress-bar-fill" style={{ height: "100%", width: `${pct}%`, background: goal.completed ? "#86EFAC" : `linear-gradient(90deg, ${color}, ${color}88)`, borderRadius: 4 }} />
                            </div>
                            <span style={{ fontSize: 12, fontWeight: 700, color: goal.completed ? "#86EFAC" : C.text, minWidth: 35 }}>{pct}%</span>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                            <span style={{ fontSize: 12, color: C.muted }}><Icon name="target" size={11} color={C.muted} /> {goal.progress}/{goal.target} {goal.unit}</span>
                            <span style={{ fontSize: 12, color: isOverdue ? "#FF6B6B" : C.muted }}><Icon name="calendar" size={11} color={isOverdue ? "#FF6B6B" : C.muted} /> {new Date(goal.deadline).toLocaleDateString("en-IN", { month: "short", day: "numeric" })}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            <button className="btn btn-ghost" style={{ padding: "7px", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}
                            ...
                            <button className="btn" style={{ padding: "7px", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(255,107,107,0.1)", color: "#FF6B6B" }}
                ...
                {filteredGoals.length === 0 && (
                  <div style={{ textAlign: "center", padding: "60px 20px", color: C.muted }}>
                    ...
                    <p style={{ fontSize: 16, fontWeight: 600 }}>No goals found</p>
                    <p style={{ fontSize: 13, marginTop: 6 }}>Try adjusting filters or create a new goal</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {activeTab === "analytics" && (
            <div style={{ animation: "fadeIn 0.4s ease" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
                {/* Monthly Trend */}
                <div className="card">
                  <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Monthly Completion Trend</h3>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 100 }}>
                    {monthlyData.map((v, i) => {
                      const months = ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"];
                      const isThis = i === new Date().getMonth();
                      return (
                        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                          <div style={{ width: "100%", borderRadius: 5, height: `${(v / 30) * 80}px`, background: isThis ? "linear-gradient(180deg, #6EE7F7, #A78BFA)" : dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)", minHeight: 6, transition: "height 0.5s ease" }} />
                          <span style={{ fontSize: 9, color: isThis ? C.accent : C.muted }}>{months[i]}</span>
                        </div>
                      );
                    })}
                  </div>
                  <div style={{ display: "flex", gap: 20, marginTop: 14 }}>
                    <div><p style={{ fontSize: 11, color: C.muted }}>Best Month</p><p style={{ fontSize: 18, fontWeight: 700, color: "#86EFAC" }}>30 goals</p></div>
                    <div><p style={{ fontSize: 11, color: C.muted }}>Avg/Month</p><p style={{ fontSize: 18, fontWeight: 700 }}>{Math.round(monthlyData.reduce((a, b) => a + b) / 12)}</p></div>
                    <div><p style={{ fontSize: 11, color: C.muted }}>This Month</p><p style={{ fontSize: 18, fontWeight: 700, color: C.accent }}>{monthlyData[new Date().getMonth()]}</p></div>
                  </div>
                </div>
                    ...
                    return catGoals.length > 0 && (
                      <div key={cat} style={{ marginBottom: 12 }}>
                        ...
                        <div style={{ height: 7, background: C.surface2, borderRadius: 4, overflow: "hidden" }}>
                          <div className="progress-bar-fill" style={{ height: "100%", width: `${pct}%`, background: color, borderRadius: 4 }} />
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="card" style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 28, marginTop: 16 }}>
                  {[
                    { label: "Total Active Days", val: Object.values(HEATMAP_DATA).filter(v => v > 0).length },
                    { label: "Most Goals in a Day", val: Math.max(...Object.values(HEATMAP_DATA)) },
              ...
              <div className="card">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
                  {PRIORITIES.map(p => {
                    ...
                    return (
                      <div key={p} style={{ textAlign: "center", padding: "16px 10px", background: C.surface2, borderRadius: 12, border: `1px solid ${colors[p]}22` }}>
                        <div style={{ position: "relative", width: 64, height: 64, margin: "0 auto 10px" }}>
                          ...
                          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: colors[p] }}>{pg.length ? Math.round((done / pg.length) * 100) : 0}%</div>
                        </div>
                        <p style={{ fontSize: 13, fontWeight: 600, color: colors[p] }}>{p}</p>
                        <p style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{done}/{pg.length} done</p>
                      </div>
                    );
                  })}
                </div>
            ...
            <div style={{ animation: "fadeIn 0.4s ease" }}>
              ...
              <div className="card" style={{ background: "linear-gradient(135deg, rgba(110,231,247,0.08), rgba(167,139,250,0.08))", border: `1px solid rgba(110,231,247,0.15)`, marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                  <div style={{ position: "relative", width: 80, height: 80 }}>
                    <CircularProgress pct={getLevelProgress(xp)} size={80} stroke={6} color="url(#grad)" />
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 24, fontWeight: 700 }}>{getLevelTitle(getLevel(xp))}</p>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
                      <div style={{ flex: 1, height: 8, background: C.surface2, borderRadius: 4, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${getLevelProgress(xp)}%`, background: "linear-gradient(90deg, #6EE7F7, #A78BFA)", borderRadius: 4, transition: "width 0.8s ease" }} />
                      </div>
                      <span style={{ fontSize: 12, color: C.muted }}>{xp % 200}/200 XP to Lv.{getLevel(xp) + 1}</span>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 32, fontWeight: 700, color: C.accent }}>{xp}</p>
              ...
              <div className="card" style={{ marginBottom: 24, background: "linear-gradient(135deg, rgba(252,211,77,0.06), rgba(253,164,175,0.06))", border: "1px solid rgba(252,211,77,0.15)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 32, fontWeight: 700, color: "#FCD34D" }}>{streak} days</p>
                    <p style={{ fontSize: 14, color: C.muted }}>Current streak · Longest: <strong style={{ color: "#FCD34D" }}>{longestStreak} days</strong></p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 12, color: C.muted }}>Next milestone</p>
                    <p style={{ fontSize: 20, fontWeight: 700, color: "#FDA4AF" }}>14 days 🎯</p>
                    <p style={{ fontSize: 11, color: C.muted }}>2 more days to go!</p>
                  </div>
                </div>
              </div>

              {/* Badges */}
              <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 18, fontWeight: 600, marginBottom: 16 }}>Badges & Awards</h3>
            ...
            <div style={{ animation: "fadeIn 0.4s ease", maxWidth: 600 }}>
              ...
              {[
                { section: "Appearance", items: [
                  { label: "Dark Mode", desc: "Easy on the eyes, great for focus", toggle: dark, onToggle: () => setDark(d => !d) },
                ]},
                { section: "Notifications", items: [
                  { label: "Daily Check-in Reminder", desc: "9:00 AM every day", toggle: true, onToggle: () => showNotif("✅ Notification setting saved!") },
                  { label: "Deadline Alerts", desc: "24h before due date", toggle: true, onToggle: () => showNotif("✅ Notification setting saved!") },
                  { label: "Streak Breaks", desc: "Alert when streak is at risk", toggle: true, onToggle: () => showNotif("✅ Notification setting saved!") },
                  { label: "Goal Completion", desc: "Celebrate every win", toggle: false, onToggle: () => showNotif("✅ Notification setting saved!") },
                ]},
                { section: "Gamification", items: [
                  { label: "XP & Levels", desc: "Earn experience points", toggle: true, onToggle: () => showNotif("✅ Setting saved!") },
                  { label: "Badges", desc: "Unlock achievement badges", toggle: true, onToggle: () => showNotif("✅ Setting saved!") },
                  { label: "Leaderboard", desc: "Compare with friends", toggle: false, onToggle: () => showNotif("🚀 Coming soon!") },
                ]},
                { section: "Advanced", items: [
                  { label: "AI Goal Suggestions", desc: "Smart recommendations based on habits", toggle: false, onToggle: () => showNotif("🤖 AI suggestions coming soon!") },
                  { label: "PWA Offline Mode", desc: "Use GoalFlow without internet", toggle: false, onToggle: () => showNotif("📱 PWA feature coming soon!") },
                  ...
                  {items.map((item, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderTop: i > 0 ? `1px solid ${C.border}` : "none" }}>
                      ...
                      <div onClick={item.onToggle} style={{ width: 44, height: 24, borderRadius: 12, background: item.toggle ? "linear-gradient(90deg, #6EE7F7, #A78BFA)" : dark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)", cursor: "pointer", position: "relative", transition: "all 0.2s" }}>
                        <div style={{ position: "absolute", top: 3, left: item.toggle ? 23 : 3, width: 18, height: 18, borderRadius: "50%", background: "#fff", transition: "left 0.2s", boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="card" style={{ background: "rgba(255,107,107,0.05)", border: "1px solid rgba(255,107,107,0.15)" }}>
                ...
                <p style={{ fontSize: 13, color: C.muted, marginBottom: 14 }}>These actions are irreversible. Please be certain.</p>
                <div style={{ display: "flex", gap: 10 }}>
                  <button className="btn" style={{ padding: "9px 16px", fontSize: 13, background: "rgba(255,107,107,0.1)", color: "#FF6B6B" }} onClick={() => showNotif("⚠️ All data cleared!")}>Clear All Goals</button>
                  <button className="btn" style={{ padding: "9px 16px", fontSize: 13, background: "rgba(255,107,107,0.1)", color: "#FF6B6B" }} onClick={() => { setScreen("login"); }}>Delete Account</button>
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(8px)", padding: 16 }}>
          <div style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 20, padding: 28, width: "100%", maxWidth: 360, animation: "fadeIn 0.3s ease" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input className="input" type="number" value={progressInput} onChange={e => setProgressInput(e.target.value)} min={0} max={showProgressModal.target} placeholder={`Max: ${showProgressModal.target}`} style={{ fontSize: 20, fontWeight: 700, textAlign: "center", height: 54 }} />
            </div>
            <div style={{ height: 8, background: C.surface2, borderRadius: 4, overflow: "hidden", marginBottom: 20 }}>
              <div style={{ height: "100%", width: `${Math.min((parseFloat(progressInput) / showProgressModal.target) * 100, 100)}%`, background: "linear-gradient(90deg, #6EE7F7, #A78BFA)", borderRadius: 4, transition: "width 0.3s" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
      ...
      <div style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 22, padding: 30, width: "100%", maxWidth: 500, animation: "fadeIn 0.3s ease", maxHeight: "90vh", overflow: "auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <textarea className="input" rows={2} placeholder="Describe your goal in detail..." value={form.desc} onChange={e => set("desc", e.target.value)} style={{ resize: "vertical", minHeight: 70 }} />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input className="input" placeholder="pages, km, hrs..." value={form.unit} onChange={e => set("unit", e.target.value)} />
            </div>
            <div>
